...
const Grocery: React.FC<triggerProps> = ({ trigger, setTrigger }) => {
    ...
    const [recipeObj, setRecipeObj] = useState<RecipeTableObj>();

    const handleLink = (url: string) => {
        window.open(url, '_blank', 'noopener,noreferrer');
    }
    
    const handleOpenModal = (recipe: RecipeTableObj) => {
        setRecipeObj(recipe);
        setIsModalOn(true);
    }
            ...
            const groceries = fetchedGroceries.data.map((recipeItem: RecipeTableObj) => {
                return (
                    <div className='w-full h-[120px] p-2'>
                        <div
                            className='flex justify-start items-center w-full h-full border-2 border-amber-600 bg-white rounded-lg hover:scale-105 ease-in duration-300 p-2 cursor-pointer'

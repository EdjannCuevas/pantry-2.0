...
const Recipes: React.FC<triggerProps> = ({ trigger, setTrigger }) => {
    ...
    useEffect(() => {
        const getRecipes = async () => {
            ...
            // const selectedObj = fetchedSelected.filter((foodItem: PantryObj) => foodItem.isSelected);
            ...
            const recipeList = fetchedRecipes.data.hits.map((item: ResponseObj) => {
                ...
                return (
                    <div className='h-[120px] w-full py-1 px-2'>
                        <div
                            className='flex bg-white shadow-xl z-50 border-2 border-amber-600 hover:scale-105 ease-in-out duration-300 cursor-pointer'
                            onClick={() => {
                                handleOpenModal({
                                    label: name,
                                    image: image,
                            ...
                            <div className='flex flex-col justify-start w-[210px] md:w-[300px]'>
                                ...
                                <div className='flex w-full h-[50px] md:h-[80px] justify-between items-center p-2'>
                                    <div className='flex flex-col justify-center items-center'>
                                        <label className='uppercase text-[10px] md:text-[13px] text-green-600'><BiTime/></label>
                                        <p className='text-[10px] md:text-[13px]'>{ cook_time } mins</p>
                                    </div>
                                    <div className='flex flex-col justify-center items-center'>
                                        <label className='text-amber-600 uppercase text-[8px] md:text-[11px]'>Servings</label>
                                        <p className='text-[10px] md:text-[13px]'>{ servings }</p>
                                    </div>
                                    <div className='flex flex-col justify-center items-center'>
                                        <label className='text-blue-500 uppercase text-[8px] md:text-[11px]'>Calories/serving</label>
        ...
        <div className='flex w-full h-16 p-2'>
            {
                ...
                <div className='min-w-[100px] h-12 m-2'>
                    <div
                        className='flex justify-center items-center w-full h-full rounded-xl rounded-l-full bg-green-500 border-white border-2 p-2 hover:scale-75 ease-out duration-500 cursor-pointer'
                        onClick={() => navigate('/')}
                    >
